...
// 操作结果：由串copy构造新串——复制构造函数
{
    ...
    strcpy_s(strVal, length + 1, copy.CStr());				// 复制串值
}
...
// 操作结果：从C风格串转换构造新串——转换构造函数
{
    ...
    strVal = new char[length + 1];				// 分配存储空间 
    strcpy_s(strVal, length + 1, copy);						// 复制串值
}
...
// 操作结果：将串addOn连接到addTo串的后面
{
    ...
    int newlen = strlen(cFirst) + strlen(cSecond) + 1;
    char *copy = new char[newlen];// 分配存储空间
    strcpy_s(copy, newlen, cFirst);						// 复制第一个串
    strcat_s(copy, newlen, cSecond);						// 连接第二个串
    addTo = copy;								// 串赋值
    delete[]copy;								// 释放copy
}
...
// 操作结果：将串original复制到串copy
{
    ...
    int newlen = strlen(cOriginal) + 1;
    char *cCopy = new char[newlen];// 分配存储空间
    strcpy_s(cCopy, newlen, cOriginal);					// 复制串
    copy = cCopy;								// 串赋值
...
// 操作结果：将串original复制n个字符到串copy 
{
    ...
    strncpy_s(cCopy, len + 1, cOriginal, n);				// 复制串
...
// 操作结果：返回串s的第pos个字符开始的长度为len的子串
{
    ...
    {	// 返回串s的第pos个字符开始的长度为len的子串
        ...
        strncpy_s(sub, len + 1, str + pos, len);			// 复制串
        sub[len] = '\0';						// 串值以'\0'结束
